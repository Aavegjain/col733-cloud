...
class MapperState:
  ...
  def __post_init__(self):
    # called after __init__. MapperState is dataclass so init generated by py
    # UDP 
    ...
    for i in range(len(self.reducer_ports)):
      ...
      while True:
        try:
          ...
          logging.info(f"{self.id}: connected to reducer_{i}") 
          break
  ...
  def reconnect_reducer_socket(self, idx):
    # tries to connect 
    conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    while True:
      try:
        conn.connect(("localhost", self.reducer_ports[idx]))
        logging.info(f"{self.id}: connected to reducer_{idx}") 
        break
      except ConnectionRefusedError:
        # Keep trying, eventually reducer will be up.
        logging.warning("Couldn't connect to reducer, waiting for it go up.")
  ...
  def to_reducer(self, idx, msg: Message):
    msg_bytes = msg.serialize() 
    while True:
      try:
        self.reducer_sockets[idx].sendall(msg_bytes) 
        break 
      except Exception as e:
        logging.error(f"{self.id} Error in sending to reducer {idx}: {e}")
        self.reducer_sockets[idx] = self.reconnect_reducer_socket(idx) 
  ...
  def checkpoint(self, checkpoint_id: int):
    logging.info(f"{self.id} performing checkpointing of its stream ID = {self.last_stream_id}")
    ...
    if (checkpoint_id < 0):
      ...
      try:
        ...
        with open(filename, 'r') as file:
          ...
          line = file.readline()
          logging.info(f"{self.id}: read {line} from filename: {filename}") 
        self.last_stream_id = line[:-1].encode() 
        logging.info(f"{self.id}: last_stream_id is {self.last_stream_id}")
      except Exception as e:
  ...
  def word_2_socket(self, word: str) -> int: 
...
class Checkpoint(Cmd):
  ...
  def send_marker(self, idx, state: MapperState):
    msg = Message(msg_type = MT.FWD_CHECKPOINT, source = state.id, checkpoint_id = state.last_cp_id, recovery_id = state.last_recovery_id)
    state.to_reducer(idx, msg) 
    

  def handle(self, state: MapperState):
    # TODO: this mapper received checkpoint marker from coordinator. Take appropriate actions.
    # 1. checkpoint and update state  2. send marker to reducer 3. send ack to coord 
    ...
    if (self.recovery_id != state.last_recovery_id):
      logging.info(f"{state.id}: ignoring msg since recovery id doesnt match, mine:{state.last_recovery_id}, recvd: {self.recovery_id}")
      return 
...
class CmdHandler(threading.Thread):
  ...
  def run(self) -> None:
    while True:
      try:
        if (self.state.is_wc_done):
          logging.info(f"{self.state.id}:  blocking queue since wc done" )
